...
'
'#Parameters :
'   p_Array [Variant]   : Source Array to be transpose
...
Public Function TransposeArray(p_Array As Variant) As Variant
...
'#Source :
'----------------------------------------------------------------------------------------------------------------------------------------------------
Public Function Join2D(ByVal vArray As Variant, Optional ByVal sWordDelim As String = " ") As Variant
    ...
    For i = LBound(vArray, 1) To UBound(vArray, 1)
        For j = LBound(vArray, 2) To UBound(vArray, 2)
            ...
            aLine(j) = IIf(IsNull(vArray(i, j)), "", vArray(i, j))
        Next j
...
'#Source :
'----------------------------------------------------------------------------------------------------------------------------------------------------
Function SortArray(ByRef p_ls As Variant, p_col As Integer) As Variant
    
    'declarations
    Dim res As Variant, tmp As Variant
    ...
    'sort
    For k = LBound(res, 1) To UBound(res, 1)
        For num = LBound(res, 1) To UBound(res, 1) - 1
            'compare
            If (res(num, p_col) > res(num + 1, p_col)) Then
                'inverse
                For col = LBound(res, 2) To UBound(res, 2)
                    tmp = res(num, col)
...
'
'#Parameters :
'   p_ls [Variant] : list it will be sorted
'   p_col1 [Integer] : first column sortedby
'
'#Return : Type
'
'#Example : call XxxYyy (Name1, Name2)
...
'#Source :
'----------------------------------------------------------------------------------------------------------------------------------------------------
'fonction qui fait un tri rapide
Sub QuickSort(ByRef p_ls As Variant, ByVal p_col1 As Integer, ByVal p_ascending As Boolean, ByVal Gauche As Long, ByVal Droite As Long, Optional ByVal p_col2 As Integer = -1)
    
    'declarations
    Dim Temp As Variant, Pivot As Variant, Pivotbis As Variant
    Dim i As Long, j As Long
    Dim k As Integer
    ...
    i = Gauche
    j = Droite
    Pivot = IIf(IsNull(p_ls((Gauche + Droite) / 2, p_col1)), "", p_ls((Gauche + Droite) / 2, p_col1))
    If (p_col2 > -1) Then Pivotbis = IIf(IsNull(p_ls((Gauche + Droite) / 2, p_col2)), "", p_ls((Gauche + Droite) / 2, p_col2))
    ReDim Temp(LBound(p_ls, 2) To UBound(p_ls, 2))
    
    'loop
    Do
        If (p_ascending) Then
            If (p_col2 = -1) Then
                While (Pivot > IIf(IsNull(p_ls(i, p_col1)), "", p_ls(i, p_col1))): i = i + 1: Wend
                While (p_ls(j, p_col1) > Pivot): j = j - 1: Wend
            Else
                While ((Pivot > IIf(IsNull(p_ls(i, p_col1)), "", p_ls(i, p_col1))) Or (Pivot = IIf(IsNull(p_ls(i, p_col1)), "", p_ls(i, p_col1)) And Pivotbis > IIf(IsNull(p_ls(i, p_col2)), "", p_ls(i, p_col2)))): i = i + 1: Wend
                While ((IIf(IsNull(p_ls(j, p_col1)), "", p_ls(j, p_col1)) > Pivot) Or (Pivot = IIf(IsNull(p_ls(j, p_col1)), "", p_ls(j, p_col1)) And Pivotbis < IIf(IsNull(p_ls(j, p_col2)), "", p_ls(j, p_col2)))): j = j - 1: Wend
            End If
        Else
            If (p_col2 = -1) Then
                While (Pivot < p_ls(i, p_col1)): i = i + 1: Wend
                While (p_ls(j, p_col1) < Pivot): j = j - 1: Wend
            Else
                While (Pivot < IIf(IsNull(p_ls(i, p_col1)), "", p_ls(i, p_col1)) Or (Pivot = IIf(IsNull(p_ls(i, p_col1)), "", p_ls(i, p_col1)) And Pivotbis < IIf(IsNull(p_ls(i, p_col2)), "", p_ls(i, p_col2)))): i = i + 1: Wend
                While (IIf(IsNull(p_ls(j, p_col1)), "", p_ls(j, p_col1)) < Pivot Or (Pivot = IIf(IsNull(p_ls(j, p_col1)), "", p_ls(j, p_col1)) And Pivotbis > IIf(IsNull(p_ls(j, p_col2)), "", p_ls(j, p_col2)))): j = j - 1: Wend
        ...
        If (j + 1 > i) Then
            'inverse
            For k = LBound(p_ls, 2) To UBound(p_ls, 2)
                Temp(k) = IIf(IsNull(p_ls(i, k)), "", p_ls(i, k))
                p_ls(i, k) = IIf(IsNull(p_ls(j, k)), "", p_ls(j, k))
                p_ls(j, k) = Temp(k)
            Next k
    ...
    'appel recursif si necessaire
    If (Gauche < j) Then Call QuickSort(p_ls, p_col1, p_ascending, Gauche, j, p_col2)
    If (i < Droite) Then Call QuickSort(p_ls, p_col1, p_ascending, i, Droite, p_col2)
